...
import { ObtenerAccidentes } from "@/api/accidentes/obtener-accidentes";
...
const AccidentePageAdmin = () => {
  const { user } = useAuthStore();
  const [limit, setLimit] = useState<number>(10);
  const [offset, setOffset] = useState<number>(0);
  const { data, isLoading, isError } = useQuery({
    queryKey: ["accidentes-nd", limit, offset],
    queryFn: () => ObtenerAccidentes(limit, offset),
    staleTime: 60 * 100 * 5,
    retry: 0,
  });
...
export default AccidentePageAdmin;
